...
#model_id_chat ='global.anthropic.claude-sonnet-4-5-20250929-v1:0'  #models["CHAT"]

##con sonnet 4.5
#model_id_chat='arn:aws:bedrock:us-east-1:552102268375:application-inference-profile/hvr5d8f7n720'

#model_id_chat = 'global.anthropic.claude-sonnet-4-6'
...
#Con un cambio en el prompt se vuelve a hacer el catching
#... versión test 123 ...


SYSTEM_PROMPT_PROCESOS_FIJO = """
...
REFORMULATE_WITH_HISTORY_PROMPT_FIJO = """
...
Genera únicamente un título breve, profesional e institucional, de máximo 50 caracteres en español, basado en una consulta relacionada con procesos administrativos.
...
BASE_CONOCIMIENTOS_PROCESOS = "JQW2MHWKBF"
...
def reformulate_question(question, history):
    ...
    messages = [
        (
            "human",
            [
                {"type": "text", "text": REFORMULATE_WITH_HISTORY_PROMPT_FIJO},
    ...
    #print("REFORMULACION USAGE METADATA:", getattr(response, "usage_metadata", None))
    return get_text_from_response(response)


def stream_procesos_model(question, history, docs):
    ...
    messages = [
        (
            "system",
            [
                {"type": "text", "text": SYSTEM_PROMPT_PROCESOS_FIJO},
                {"cachePoint": {"type": "default", "ttl": "1h"}},
                {
                    "type": "text",
...
def run_procesos_chain(question, history, codigos_activos):
    chain_parts = build_procesos_chain(codigos_activos)
    ...
    for chunk in stream_procesos_model(
        ...
        if chunk.get("usage_metadata"):
            ...
            #print("PROMPT PRINCIPAL PROCESOS USAGE METADATA:", final_usage_metadata)
...
def generate_name(prompt):
    try:
        ...
        #print("GENERATE_NAME USAGE METADATA:", getattr(response, "usage_metadata", None))
        return get_text_from_response(response)
